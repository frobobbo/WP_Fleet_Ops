...
- Fail-closed combined dashboard at `/`, excluding stale fleet and care observations from headline health metrics while retaining clearly labeled observed values for investigation, surfacing incomplete paired evidence in the status banner, exposing the exact deployed revision for release correlation, plus Markdown reports at `/report`.
