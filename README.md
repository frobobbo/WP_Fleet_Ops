...
- Fail-closed dashboard and client account rollups at `/api/summary` and `/api/clients`, excluding stale fleet snapshots and care checks from current scores, risk counts, health counts, and critical-alert totals while retaining monitoring-gap visibility. The summary reports stale, invalid-timestamp, and clock-skew evidence separately for both observation types, returns a null current average when no current snapshots exist while retaining a labeled observed average, and excludes invalid or future-dated timestamps from its latest-snapshot marker.
