...
- Fail-closed site inventories at `/api/sites` and `/api/site-directory`, reporting stale, invalid, or future-dated snapshot health as unknown, suppressing untrusted alerts from the current `alerts` field, and retaining explicitly labeled observed status and alerts for investigation.
