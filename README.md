...
- Fail-closed site inventory at `/api/sites`, requiring current paired fleet-snapshot and care-check evidence before publishing status or alerts, reporting each source's freshness, and retaining explicitly labeled observations for investigation. The broader `/api/site-directory` includes sites missing their first snapshot and reports stale, invalid, or future-dated snapshot health as unknown.
