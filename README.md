...
- Bounded site identifiers (200-character site/client labels and 2,048-character URLs) plus 100-character WordPress version values reject accidental oversized submissions before they can inflate persistent dashboard and report data; non-printing site/client labels and URL characters, malformed DNS hosts, malformed percent escapes, percent-encoded controls, empty ports, ambiguous raw-backslash URLs, and legacy numeric IPv4 URLs are rejected, semicolon path parameters are preserved, and equivalent expanded and compressed IPv6 host spellings are canonicalized to one tracked endpoint.
