...
- Site/client registry with SQLite persistence; equivalent percent-escape hex casing is canonicalized to prevent duplicate site identities, and sites can be removed with a `DELETE /sites?url=<url>` call that cascades to all care checks and fleet snapshots.
- Bounded site identifiers (200-character site/client labels and 2,048-character URLs) plus normalized 100-character WordPress version values reject accidental oversized submissions before they can inflate persistent dashboard and report data; version labels are trimmed, blank values use the `unknown` sentinel, and non-printing site/client/version labels and URL characters are rejected. Malformed DNS hosts, malformed percent escapes, percent-encoded controls, empty ports, ambiguous raw-backslash URLs, and legacy numeric IPv4 URLs are also rejected, semicolon path parameters are preserved, and equivalent expanded and compressed IPv6 host spellings are canonicalized to one tracked endpoint.
