...
- Paired monitoring coverage at `/api/monitoring-coverage`, reporting per-site fleet snapshot and care-check freshness plus the minimum evidence capture needed to restore coverage, so integrations can close gaps without rerunning evidence that is already current; `/api/dispatch-summary` and `/api/daily-ops-brief` route either evidence gap to operators. Use `?client=<name>` (or `Unassigned`) for an account-scoped audit, with unknown accounts returning `404` instead of an empty healthy result.
