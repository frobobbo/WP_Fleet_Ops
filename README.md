...
- Paired monitoring coverage at `/api/monitoring-coverage`, reporting per-site fleet snapshot and care-check freshness so integrations can restore missing or stale evidence without parsing report text; use `?client=<name>` (or `Unassigned`) for an account-scoped audit.
